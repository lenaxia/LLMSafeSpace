...
func TestStartInformers(t *testing.T) {
	...
	go func() {
		...
		time.Sleep(metav1.Duration{Duration: metav1.Duration{Duration: metav1.Duration{Duration: metav1.Duration{Duration: metav1.Duration{Duration: 100 * time.Millisecond}}}}})
		close(stopCh)
	}()

	// Wait for informers to start and stop
	time.Sleep(metav1.Duration{Duration: metav1.Duration{Duration: 200 * time.Millisecond}})
