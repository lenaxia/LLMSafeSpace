...
Concrete failure modes, all validated:

1. **Stale model picker after credential bind (≤5s):** Replica A evicts cache on
   credential bind; Replica B still has stale list (new provider absent) for ≤5s.
...
2. **Stale `relayInjected` during Phase 1→Phase 2 (≤20s):** 5s model cache TTL +
   ...
   and send a wrong per-request override. **Validated live:** opencode 1.15.12 ignores
   overrides to `disabled_providers` entries, falls back to session model
   (`opencode-relay/big-pickle`). Inference routes correctly despite the stale override.

3. **Stale `Selected` indicator:** `annotatedModel.Selected` is set from DB
   (`GetDefaultModel`) after cache lookup — never in the cache. Always fresh.
