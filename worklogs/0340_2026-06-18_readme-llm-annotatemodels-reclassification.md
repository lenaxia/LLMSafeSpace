...
2. **annotateModels section rewrite** (lines 517-519): retitled "dead code (tech debt to remove)" → "intentional defense-in-depth". New body:
   ...
   - References worklog 0338 for the full rationale
...
2. **Referenced worklog 0338 rather than re-deriving the argument.** The README is high-level; the detailed defense-in-depth reasoning lives in 0338 (and US-46.2's scope section). The README points there rather than duplicating ~100 lines of analysis.
...
4. **Used worklog number 0340, not 0338.** Origin/main currently has *two* worklogs numbered 0338 (a collision between PR #225 and a parallel PR — `0338_us-46.2-keep-annotatemodels-guard.md` and `0338_migration-safety-ci-parity.md`). This is a separate bug in the repolint sequence check (it should catch this) and is flagged below; this PR avoids the collision by jumping to 0340.
...
| Worklog 0340 is the next available number | Verified via `git ls-tree origin/main worklogs/` — highest on main is 0339; the duplicate 0338 entries mean 0340 is the safe next |
...
**Worklog numbering collision on origin/main.** Two distinct worklogs share number 0338:
- `worklogs/0338_2026-06-18_migration-safety-ci-parity.md`
- `worklogs/0338_2026-06-18_us-46.2-keep-annotatemodels-guard.md`

This indicates the `repolint` sequence check did not catch the collision. Likely cause: both PRs were open against main simultaneously, each passed repolint against its own base (which had different highest numbers), and the merge order was not re-validated. This is a real bug in the CI pipeline that should be tracked separately — a non-fast-forward merge of one PR against the updated main should have re-run repolint and caught the collision. Flagging for a future hardening story; not fixing here (out of scope for a README edit).
...
2. (Separate) File an issue for the worklog 0338 collision / repolint merge-order gap.
...
| File | Action |
|------|--------|
| `README-LLM.md` | Lines 5-6 (version/date), 517-519 (annotateModels section rewrite + predicate correction), 1421 (new Version History entry) |
| `worklogs/0340_2026-06-18_readme-llm-annotatemodels-reclassification.md` | Created (this file) |
