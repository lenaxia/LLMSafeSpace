...
export function RelaySetupWizard({ onComplete }: { onComplete?: () => void }) {
  ...
  const handleSaveAWS = async () => {
    try {
      await relayApi.saveAWSConfig(awsConfig);
      toast("AWS config saved");
      await load();
    } catch (e) {
      toast(e instanceof Error ? e.message : "Failed to save AWS config", "error");
    }
  };
  ...
  const handleTestAWS = async () => {
    setTesting(true);
    try {
      const result = await relayApi.testAWS();
      if (result.valid) {
        toast(`AWS connection valid (Account: ${result.accountId})`);
      } else {
        toast(`AWS connection failed: ${result.error || "unknown"}`, "error");
      }
    } catch (e) {
  ...
  const handleDownloadCA = async () => {
    try {
      const blob = await relayApi.downloadCA();
      const url = URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = url;
      a.download = "relay-root-ca.pem";
      a.click();
      URL.revokeObjectURL(url);
    } catch (e) {
      toast(e instanceof Error ? e.message : "Failed to download CA", "error");
    }
  };
  ...
  const prerequisites = [
    { label: "cert-manager installed", ok: setup.certManagerInstalled },
    ...
    },
    {
      ...
      content: (
        <div className="space-y-3">
          <button onClick={handleDownloadCA} className="flex items-center gap-2 rounded-md bg-accent px-3 py-1.5 text-sm hover:bg-accent/80">
            <Download className="h-4 w-4" /> Download root CA cert
          </button>
          <div className="space-y-2">
            <input
              className="w-full rounded-md border border-border bg-background px-3 py-1.5 text-sm"
              placeholder="Trust Anchor ID (ta-xxxxx)"
              value={awsConfig.trustAnchorId}
              onChange={(e) => setAWSConfig({ ...awsConfig, trustAnchorId: e.target.value })}
            />
            <input
              className="w-full rounded-md border border-border bg-background px-3 py-1.5 text-sm"
              placeholder="Profile ID (p-xxxxx)"
              value={awsConfig.profileId}
              onChange={(e) => setAWSConfig({ ...awsConfig, profileId: e.target.value })}
            />
            <input
              className="w-full rounded-md border border-border bg-background px-3 py-1.5 text-sm"
          ...
          <div className="flex gap-2">
            <button onClick={handleSaveAWS} disabled={!awsConfig.trustAnchorId} className="rounded-md bg-primary px-3 py-1.5 text-sm text-primary-foreground disabled:opacity-50">
              Save Config
            </button>
            <button onClick={handleTestAWS} disabled={!setup.awsConfigured || testing} className="rounded-md border border-border px-3 py-1.5 text-sm disabled:opacity-50">
              {testing ? "Testing..." : "Test Connection"}
            </button>
          </div>
          {setup.awsConfigured && <span className="text-sm text-green-500">AWS configured</span>}
        </div>
      ),
    },
    {
      ...
      content: (
        <div className="space-y-2">
          ...
          <button onClick={handleSaveOCI} disabled={!ociCreds.tenancy} className="rounded-md bg-primary px-3 py-1.5 text-sm text-primary-foreground disabled:opacity-50">
            Save & Test
          </button>
          {setup.ociConfigured && <span className="text-sm text-green-500">OCI configured</span>}
        </div>
      ),
    },
    {
