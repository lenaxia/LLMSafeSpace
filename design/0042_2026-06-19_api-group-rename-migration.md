...
### Phase 3: detach PVCs from old CRs, re-attach to new CRs

For each PVC:

1. **Read** `pvc.metadata.ownerReferences` — there is one ref pointing
   at the singular-group Workspace CR.
2. **Patch** to remove that ownerReference. The PVC is now ownerless;
   `pvc-protection` finalizer keeps it alive.
...
4. **Patch** the PVC to add an ownerReference pointing at the new
   plural-group CR (using its newly-assigned UID).
5. **Verify** the PVC's `metadata.ownerReferences[0]` matches the new CR
   exactly.

This phase is idempotent: running it twice on the same PVC is a no-op
because (a) the new CR already exists, (b) the PVC already has the new
ownerRef, (c) the patch is a strategic merge that converges.
...
1. For each `llmsafespace.dev/v1` Workspace CR:
   ...
   - **Delete** the CR.
2. The cascade *does not* propagate to PVCs because we already removed
   the ownerReference in Phase 3. PVCs survive.
...
| 3 | Patch of PVC ownerReference fails (e.g. apiserver throttling) | Retry; idempotent. If sustained, restore old ownerRef and abort. |
...
re-add ownerRefs to the PVCs pointing at those CRs, restart the old
controller. Painful but possible if a backup exists.
...
This backup is the only thing standing between us and total data loss
if Phase 3 goes wrong. It must be verified non-empty before proceeding.
