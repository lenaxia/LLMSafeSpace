...
| 44 | Session Reliability & Transparency — terminal SSE events on agent death, session-aware restart, OOM detection, memory pressure warnings, request buffering, fix api-key restart bug | ❌ Not Started | Epics 3, 6, 27a. **Driven by 2026-06-16 production incidents (OOMKill + unsafe restart). 11 user stories across 3 phases (~23 days).** |
