...
| 29 | Handler Decomposition & Agent Client Abstraction | 🔶 Partial | **US-29.4 ✅ shipped** (worklog 0343): `WorkspaceEnvHandler` extracted from `SecretsHandler`; 19 unit tests + e2e passing. **US-29.7 ✅ shipped**: contract test asserting all opencode-proxied routes send Basic auth (13 routes). Can proceed with US-29.1 (AgentClient interface), US-29.2/29.3/29.5 (further splits), US-29.6 (auth-enforcing mocks), US-29.8 (constructor injection). |
