...
| 23 | Controller Race Hardening | 🔶 Partial | Stories 1+4 ✅ shipped. **`WorkspaceStatusUpdateConflictsTotal` ✅ shipped** + `recordStatusUpdateConflictOnError` helper at 18 sites. Stories 2+3 remain — design specs exist (`US-23.2`, `US-23.3`). Story 3 (single-writer migration) must ship before Story 2 (retry-on-conflict) per R4. Story 3 requires: CRD migration (`Spec.Suspend`), `LastActivityAt` → annotation, API stops writing `Status.Phase`. ~2-3 day effort. |
...
| 41 | Message Queue Reliability — fix streaming state clear timing, add 409 guard for in-flight sessions, restore dead `onSessionIdle` activity recording | 🔶 Partial | `msgqueue/service.go` implemented (`Dequeue`, `Requeue`, `PeekAll`, `PeekAllWorkspace`); wired in `proxy_events.go`, `proxy_handlers.go`. Queue lifecycle behaviors addressed (worklog 0290, 0301, 0309). **Remaining**: verify 409 guard completeness for all in-flight session paths. |
