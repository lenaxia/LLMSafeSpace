...
**Last audited:** 2026-06-05 (code-verified; worklogs used as navigation hints, not source of truth)
...
| 12 | Usage Metering & Billing | ❌ Not Started | No implementation. Correct sequencing: Epic 30 first (stable provider routing needed for LLM metering). US-12.12/13/14 are independent and can start anytime. |
| 13 | Settings Enforcement | 🔶 Partial | **US-13.3 gap**: `applyWorkspaceDefaults` never sets `crd.Spec.MaxActiveSessions`; proxy uses hardcoded fallback of 5 — admin-configured session cap is silently ignored. Two-line fix. **US-13.15**: Comment stub at `workspace_service.go:214` is correct — Epic 30 US-30.3 implements this as `CredentialProvisioner` with the new `credential_auto_apply` table. **US-13.10**: `ModelSelector` does not read `preferredModel` user setting. |
...
| 17 | Security Review & Penetration Testing | 🔶 Partial | Pentest + ~46 code fixes complete. Open: post-remediation live re-pentest not run (no `phase-{2..7}-postfix` dirs); **F1.7.2** (API keys plaintext) and **G25** (secret in logging middleware) are both HIGH severity and classified as OTHER agent's branch; **RT-7.9** (XSS corpus) — `rehype-sanitize` is present, test corpus unwritten. Epic 30 threat model update needed before US-30.1. |
| 18 | Hot Migration — zero-downtime pod replacement | 🔶 Partial | **S18.10** complete. **S18.11**: readyz gate decoupled ✅ (primary goal done); `WorkspaceConditionProviderReady` condition — defer until after Epic 30: Epic 30 makes "no provider" an exceptional state (misconfiguration) rather than transient startup noise; current `AgentHealthy` condition already surfaces provider issues via `HealthBanner`; implementing a separate condition before Epic 30 stabilizes the injection pipeline is premature. Post-Epic 30: add condition + narrow the regex-parse in `agentHealthFromConditions`. S18.1–S18.9 not started — measured resume is ~17s p99, not 2min; low urgency. |
...
| Epic 12: Usage Metering & Billing | Depends on Epic 30 (provider routing); US-12.12/13/14 are independent and can start sooner |
...
```
Epic 30 (Unified Credential Model)           ← next, critical path
  ├─ Threat model update before US-30.1      ← Epic 17 gate
  ├─ Verify ConfigProviderPlugin behavior     ← US-30.4 gate
  ├─ Epic 13 US-13.15 comment stub            ← filled by Epic 30 US-30.3
  └─ Epic 27b utility (bulk reload)           ← more meaningful after Epic 30
...
After Epic 30:
  ...
  ├─ Epic 08 US-8.5 (CredentialsAvailable condition)
