...
| **G18** | **JWT revocation is broken (cache key mismatch)** | High | 🟢 **Fixed (worklog 0078)** | Pre-fix: `auth.go:203` wrote `token:<jti>`; `auth.go:270` read `token:<hash(token)>` — keys never collided | **Fixed**: `RevokeToken` now writes BOTH `token:<hash>` (visible to ValidateToken's fast-path) AND `token:<jti>` (defense-in-depth). `ValidateToken` checks `token:<jti>` after parsing claims so revocation is observed even when hash-cache entry is evicted. Regression: 6 tests in `auth_revocation_test.go`. Mutation-validated. |
