...
2. **Per-VM token auth (`X-Relay-Token`) is the security boundary.** No PKI,
   no cert-manager, no TLS certificates. Per-VM tokens are generated by the
   controller (`crypto/rand`), stored in K8s Secrets, and never exposed in
   the UI. WireGuard was removed in worklog 0447.
