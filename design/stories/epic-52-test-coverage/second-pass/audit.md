...
| US-52.6 | 9 | 7 | 1 | 1 | 8 |
| US-52.7 | 20 | 17 | 2 | 1 | 19 |
| US-52.8 | 19 | 17 | 2 | 0 | 19 |
| US-52.9 | 25 | 22 | 2 | 1 | 24 |
| US-52.10 | 24 (22 scenarios + 1 aggregator + 1 SDK-port) | 20 | 4 | 0 | 24 |
| US-52.11 | 5 journeys | 3 | 2 | 0 | 5 |
| **Total** | **209 tests + 24 canary + 5 journeys** | — | **28** | **7** | **202 tests + 24 canary + 5 journeys** |

Net reduction: 7 tests dropped (3.0%), 28 strengthened (13%). The
...
**Arithmetic check:** Per-story Planned sums to 238 items (209 tests +
24 canary + 5 journeys). KEEP + REWORK + DROP = 238 for every row.
Net = Planned − DROP = 231 items (202 tests + 24 canary + 5 journeys).
...
#### DROP-7.1: `TestE2E_Lifecycle_CRDs_Installed` (reclassified from REWORK-7.1)
...
#### REWORK-10.4: D-MCP-LONG-MESSAGE → D-MCP-BODY-LIMIT (reclassified from DROP-10.1)
...
| DROP-7.1 (CRDs smoke) | ✅ Confirmed + moved to precondition (reclassified from REWORK — test was removed, not modified) |
...
The epic now contains **202 tests + 24 canary items + 5 synthetic
journeys** across:
