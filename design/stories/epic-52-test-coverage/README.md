...
| A3 | `local/test.sh` is the only existing system-level e2e runner; CI's `e2e-pr.yml` "Full-stack E2E" job is disabled (`if: false`) | Verified: `.github/workflows/e2e-pr.yml:69` and `local/test.sh:1` |
