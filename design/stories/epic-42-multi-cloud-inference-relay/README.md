...
| A23 | The `public` anonymous key authorizes inference on opencode.ai/zen | ❌ DISPROVED (2026-06-19) — **BLOCKS Epic 42 as-designed** | **New finding (worklog 0420).** `public` returns **401 "Missing API key"** on `/chat/completions` (and `/responses`) from **both** a residential IP (24.18.52.209) **and** an AWS IP (13.221.103.1), via `Authorization: Bearer`, `x-api-key`, `api-key`, and no-auth — identical 401 in ~0.2s (application-layer auth rejection, not an IP block). The same key still returns **200** on `/models` (read-only listing). Both relays forward headers **unchanged** (`workers/inference-relay/src/index.ts:64`, `cmd/relay-proxy/proxy.go:134`) — no key injection — so a relay forwarding `Bearer public` gets 401 on inference **regardless of source IP**. This contradicts A0's premise ("the `public` key works from a residential IP"). Trajectory: 2026-06-07 `public`→429 `FreeUsageLimitError` (key accepted, quota exhausted); 2026-06-13 owner said it worked residentially; 2026-06-19 `public`→401 everywhere (key now scoped to listing). **The relay-forwarding-`public` architecture cannot produce inference.** Operator decision pending: (1) inject a real Zen key into the relay [reopens the secret-on-relay-VM threat model Epic 42 avoided], (2) re-point the relay at an operator-owned gateway [`ai.thekao.cloud` + a real key returns 200], or (3) confirm with Zen/opencode whether `public` inference was intentionally retired. |
