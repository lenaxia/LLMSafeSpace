# `charts/llmsafespace/migrations/` — Helm-bundled copy of database migrations
...
read files outside `charts/llmsafespace/`. So the migrations have to
...
```bash
make chart-sync-migrations    # cp -a api/migrations/. charts/llmsafespace/migrations/
```
