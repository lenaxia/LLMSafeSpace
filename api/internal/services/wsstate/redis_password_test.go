...
func TestRedisStore_GetCachedPassword_Miss_ReturnsFalse(t *testing.T) {
	...
	_, ok := store.GetCachedPassword(context.Background(), "ws-1")
...
func TestRedisStore_SetThenGetCachedPassword_RoundTrips(t *testing.T) {
	...
	store.SetCachedPassword(context.Background(), "ws-1", "hunter2")

	pw, ok := store.GetCachedPassword(context.Background(), "ws-1")
	require.True(t, ok)
...
func TestRedisStore_SetCachedPassword_OverwritesPreviousValue(t *testing.T) {
	...
	store.SetCachedPassword(context.Background(), "ws-1", "old")
	store.SetCachedPassword(context.Background(), "ws-1", "new")
...
func TestRedisStore_GetCachedPassword_DifferentWorkspaces_Isolated(t *testing.T) {
	...
	store.SetCachedPassword(context.Background(), "ws-1", "alpha")
	store.SetCachedPassword(context.Background(), "ws-2", "beta")

	pw1, ok1 := store.GetCachedPassword(context.Background(), "ws-1")
	require.True(t, ok1)
	assert.Equal(t, "alpha", pw1)
	pw2, ok2 := store.GetCachedPassword(context.Background(), "ws-2")
	require.True(t, ok2)
...
func TestRedisStore_InvalidatePassword_RemovesEntry(t *testing.T) {
	store, mr, _, cleanup := setupRedisStore(t)
	defer cleanup()
	store.SetCachedPassword(context.Background(), "ws-1", "pw1")
	store.SetCachedPassword(context.Background(), "ws-2", "pw2")

	store.InvalidatePassword(context.Background(), "ws-1")
	...
	pw, ok := store.GetCachedPassword(context.Background(), "ws-2")
...
func TestRedisStore_InvalidatePassword_OnMissingEntry_IsNoOp(t *testing.T) {
	...
	store.InvalidatePassword(context.Background(), "ws-never-was")

	_, ok := store.GetCachedPassword(context.Background(), "ws-never-was")
	assert.False(t, ok)
}
...
func TestRedisStore_CachedPassword_TTLAutoExpires(t *testing.T) {
	store, mr, _, cleanup := setupRedisStore(t)
	defer cleanup()
	store.SetCachedPassword(context.Background(), "ws-1", "oldpw")
