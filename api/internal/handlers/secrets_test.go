...
func TestHandler_CreateSecret_Success(t *testing.T) {
	...
	body := `{"name":"my-key","type":"llm-provider","value":"sk-secret","metadata":{"provider":"openai"}}`
	...
	if resp.Type != secrets.SecretTypeLLMProvider {
...
func TestHandler_CreateSecret_Duplicate(t *testing.T) {
	...
	body := `{"name":"dup","type":"llm-provider","value":"v1","metadata":{"provider":"x"}}`
...
func TestHandler_UpdateSecret(t *testing.T) {
	...
	// Create
	createBody := `{"name":"updatable","type":"llm-provider","value":"old","metadata":{"provider":"x"}}`
...
func TestHandler_DeleteSecret(t *testing.T) {
	...
	// Create
	createBody := `{"name":"deletable","type":"llm-provider","value":"v","metadata":{"provider":"x"}}`
	createReq := httptest.NewRequest(http.MethodPost, "/api/v1/secrets", bytes.NewBufferString(createBody))
	createReq.Header.Set("Content-Type", "application/json")
	cw := httptest.NewRecorder()
	router.ServeHTTP(cw, createReq)
...
func TestHandler_Bindings(t *testing.T) {
	...
	// Create a secret
	createBody := `{"name":"bindable","type":"llm-provider","value":"v","metadata":{"provider":"x"}}`
	createReq := httptest.NewRequest(http.MethodPost, "/api/v1/secrets", bytes.NewBufferString(createBody))
	createReq.Header.Set("Content-Type", "application/json")
	cw := httptest.NewRecorder()
	router.ServeHTTP(cw, createReq)
...
func TestHandler_AuditLog(t *testing.T) {
	...
	body := `{"name":"audited","type":"llm-provider","value":"v","metadata":{"provider":"x"}}`
	req := httptest.NewRequest(http.MethodPost, "/api/v1/secrets", bytes.NewBufferString(body))
	req.Header.Set("Content-Type", "application/json")
	w := httptest.NewRecorder()
	router.ServeHTTP(w, req)
