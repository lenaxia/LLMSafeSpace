/**
 * Chat slash command dispatch for the @llmsafespaces chat participant.
 *
...
async function handleStatus(ctx: ChatCommandContext): Promise<ChatCommandResult> {
  ...
  try {
    ...
    return {
      handled: true,
      markdown: "**Error:** Could not reach LLMSafeSpaces. Run `LLMSafeSpaces: Configure` first.\n\n```\n" + errMessage(e) + "\n```",
    };
  }
  if (workspaces.length === 0) {
    return { handled: true, markdown: "_No workspaces yet._ Use **LLMSafeSpaces: Create Workspace** to make one." };
  }
...
async function handleNewSession(ctx: ChatCommandContext): Promise<ChatCommandResult> {
  ...
  if (!active) {
    return { handled: true, markdown: "_No active workspace. Activate one first (LLMSafeSpaces: Activate Workspace)._" };
  }
