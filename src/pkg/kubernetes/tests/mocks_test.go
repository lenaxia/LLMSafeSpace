...
func TestMockSandboxOperations(t *testing.T) {
	...
	// Add this to test List as well
        listResult, err := client.LlmsafespaceV1().Sandboxes("test-namespace").List(metav1.ListOptions{})
        assert.NoError(t, err)
        assert.Len(t, listResult.Items, 2)  // Expecting 2 items as set up in SetupListMock
